...
const proc = child_process.spawnSync("ffmpeg", ["-version"], {
  encoding: "utf8"
});
...
const onOffColorTemperatureLight = {
  ...
  properties: [on(), colorTemperature()],
  actions: [],
  events: []
};
...
const dimmableColorLight = {
  ...
  properties: [color(), brightness(), on()],
  actions: [],
  events: []
};
      ...
      metadata: {
        ...
        input: {
          ...
          properties: {
            ...
            enumInput: {
              type: "string",
              enum: ["enum string1", "enum string2", "enum string3"]
            }
          }
        }
      }
          ...
          switch (descr.type) {
            ...
            case "string": {
              if (descr["@type"] === "ColorProperty") {
                ...
                value = `#${randomComponent()}${randomComponent()}${randomComponent()}`;
              } else {
...
class VirtualThingsDevice extends Device {
  ...
  performAction(action) {
    ...
    switch (action.name) {
      case "basic":
        this.eventNotify(
          new Event(this, "virtualEvent", Math.floor(Math.random() * 100))
        );
        break;
      case "trigger": {
        ...
        this.eventNotify(new Event(this, "alarmEvent", "Something happened!"));
        break;
      }
      case "silence": {
        const prop = this.properties.get("alarm");
...
class VirtualThingsAdapter extends Adapter {
  ...
  startTranscode() {
    ...
    if (ffmpegMajor >= 4) {
      args.push("-streaming", "1", "-hls_playlist", "1");
    }

    if (ffmpegMajor > 4 || (ffmpegMajor === 4 && ffmpegMinor >= 1)) {
      args.push("-seg_duration", "2", "-dash_segment_type", "mp4");
    }
